...
// Define the new structure for our configuration
export interface IConfig {
  github: {
    pat_token: string;
    owner?: string; // Owner is optional, can be parsed from remote
    default_target_branch: string;
  };
  reviewers: string[]; // For GitHub, these will be usernames
  teams_webhook_url?: string;
}
...
export function loadConfig(): IConfig {
  if (!fs.existsSync(configPath)) {
    console.error("❌ Configuration file 'config.json' not found. Please run the 'setup' command first.");
    process.exit(1);
  }
  const rawData = fs.readFileSync(configPath, 'utf-8');
  return JSON.parse(rawData) as IConfig;
